...
class RecordBFEncoding():
  ...
  def __init__(self, attr_encode_tuple_list, bf_harden=None, random_seed=42):
  ...
  def encode(self, attr_val_list, bf_harden, salt_str_list=None, mc_harden_class=None):
    """Encode values in the given 'attr_val_list' according to the settings
    ...
    if (self.random_seed != None):
      ...
      if (bf_harden=='balance'):
        BFH = hardening.Balancing()
        try:
          perm_rbf_bf, rbf_q_gram_pos_dict = BFH.harden_bf(perm_rbf_bf, rbf_q_gram_pos_dict)
        except Exception as e:
          perm_rbf_bf = BFH.harden_bf(perm_rbf_bf)

      elif(bf_harden=='fold'):
        BFH = hardening.Folding()
        try:
          perm_rbf_bf, rbf_q_gram_pos_dict = BFH.harden_bf(perm_rbf_bf, rbf_q_gram_pos_dict)
        except Exception as e:
          perm_rbf_bf = BFH.harden_bf(perm_rbf_bf)
